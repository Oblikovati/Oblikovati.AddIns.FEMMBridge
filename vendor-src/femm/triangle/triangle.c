...
/* PORT (Oblikovati FEMM bridge, mac/linux): FEMM added an unconditional
 * <windows.h> here; with NO_TIMER set below it is only needed on Windows.
 * Guard it so the mesher builds with clang/gcc on macOS and Linux. */
#ifdef _WIN32
#include <windows.h>
#endif
...
void syntax()
{
	/* PORT (mac/linux): FEMM replaced the upstream help text with a Windows
	 * ShellExecute to open the switch reference in a browser. Print the URL on
	 * non-Windows instead of pulling in the Win32 shell API. */
#ifdef _WIN32
	ShellExecute(NULL, "open", "https://www.cs.cmu.edu/~quake/triangle.switch.html", NULL, NULL, SW_SHOWNORMAL);
#else
	printf("Triangle switch reference: https://www.cs.cmu.edu/~quake/triangle.switch.html\n");
#endif
	triexit(0);
}
...
void info()
{
	/* PORT (mac/linux): see syntax() above — print the URL instead of ShellExecute. */
#ifdef _WIN32
